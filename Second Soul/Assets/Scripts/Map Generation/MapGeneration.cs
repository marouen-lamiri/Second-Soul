        ...
        for (int i = 0; i < mapSizeX; i++){
            for (int j = 0; j < mapSizeZ; j++){
				if (map[i, j] != emptySlotIdentifier && map[i - 1, j] != emptySlotIdentifier && map[i, j - 1] != emptySlotIdentifier && map[i, j] != charIdentifier && map[i, j] != EnemyIdentifier && map[i - 1, j] != EnemyIdentifier &&
                    ...
                    playerStartPositionVector3 = new Vector3(i * 10 + offset, 0.08f, j * 10 + offset);
    ...
    int[,] createDungeonHalls(int[,] map, int sizeX, int sizeZ, int numberRooms)
    {
        ...
        for (int i = 0; i < numberRooms; ++i)
        {
            ...
            do
            {
                ...
                if (map[x, z] == i + 1)
                {//because rooms start as 1. 0 is empty squares
                    goodPosition = true;
        ...
        for (int i = 0; i < mapSizeX; i = Random.Range(i, i + 3))
        {
            for (int j = 0; j < mapSizeZ; j = Random.Range(j, j + 3))
            {
                if (map[i, j] != 0 && map[i - 1, j] != 0 && map[i, j - 1] != 0 && map[i - 1, j - 1] != 0 && map[i + 1, j + 1] != 0 && nbrObstacles != 0
                    ...
                    && nbrObstaclesByRoom != 0 && previousRoomNumber != map[i, j] && map[i, j] != charIdentifier && map[i, j] != EnemyIdentifier && map[i - 1, j] != EnemyIdentifier
        ...
        for (int i = 0; i < sizeX; i++)
        {
            for (int j = 0; j < sizeZ; j++)
            {
                ...
                if (modifiedMap[i, j] == 1)
                {
                    ...
                    {  // Loop while there are coordinates to test 
